-- Slot-level prediction dataset for hot/cold prediction (v004)
...
WITH
  ShiftCapacityPerDay AS (
    WITH
      date_series AS (
        ...
        FROM
          UNNEST(GENERATE_DATE_ARRAY(CURRENT_DATE(), DATE_ADD(CURRENT_DATE(), INTERVAL 365 DAY))) AS DATE
      ),
    ...
    FROM shift_util
    WHERE DATE(DATE) BETWEEN CURRENT_DATE() AND DATE_ADD(CURRENT_DATE(), INTERVAL 365 DAY) AND venue_key_id = 14301284
  ),
  ...
  -- Prefilter reservations to reduce scanned data for current bookings
  ReservationsFiltered AS (
    ...
    WHERE venue_key_id = 14301284
      ...
      AND marked_time >= TIMESTAMP_ADD(CURRENT_TIMESTAMP(), INTERVAL -400 DAY)
      AND marked_time <= TIMESTAMP_ADD(CURRENT_TIMESTAMP(), INTERVAL 400 DAY)
  ),
  ...
  CurrentBookings AS (
    SELECT
      ...
      sc.shift_category,
      sc.slot_index,
      SUM(ra.max_guests) AS covers_as_of_date
...
SELECT
  ...
  DATE_DIFF(DATE(sc.shift_date), CURRENT_DATE(), DAY) AS day_prior,
  COALESCE(cb.covers_as_of_date, 0) AS covers_as_of_date
FROM SlotCapacity sc
LEFT JOIN CurrentBookings cb
  ON cb.shift_date = sc.shift_date
  AND cb.venue_key_id = sc.venue_key_id
  AND cb.persistent_id = sc.persistent_id
  AND cb.slot_index = sc.slot_index
