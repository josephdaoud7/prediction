...
WITH
  ...
  -- CTE to calculate the final outcome (is_hot) for each historical shift
  ShiftCapacityAndOutcome AS (
    ...
    LEFT JOIN (
        ...
        WHERE status NOT IN ('CANCELED') AND venue_key_id = 14301284
        GROUP BY 1, 2, 3
    ) AS r ON s.venue_key_id = r.venue_key_id AND s.persistent_id = r.shift_persistent_id AND s.DATE = r.reservation_date
    WHERE DATE(s.DATE) < CURRENT_DATE() AND s.venue_key_id = 14301284
  ),

  -- Create a number series for our "days prior" snapshots
  DaysPrior AS (
    SELECT day_prior FROM UNNEST(GENERATE_ARRAY(0, 365)) as day_prior
  ),

  -- Create the training "scaffold". For each historical shift, create 366 rows
  TrainingScaffold AS (
    SELECT
    ...
    WHERE s.util_capacity IS NOT NULL AND s.util_capacity > 0
  ),

  -- Aggregate reservations by the day they were created.
  ReservationsByCreationDate AS (
    ...
    FROM `sevenrooms-datawarehouse.sevenrooms_datawarehouse_prod.nightloop_ReservationActual_Latest`
    WHERE status NOT IN ('CANCELED') AND venue_key_id = 14301284 AND created IS NOT NULL
    GROUP BY 1, 2, 3, 4
  )

-- Final Step: Join the scaffold with the daily created reservations and calculate cumulative covers
SELECT
  t.shift_date,
  t.venue_key_id,
  t.persistent_id,
  ...
  -- Use a window function to get the cumulative sum of covers up to the snapshot date
...
ORDER BY
  t.shift_date, t.snapshot_timestamp;
